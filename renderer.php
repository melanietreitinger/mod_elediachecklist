...
use mod_elediachecklist\local\progress_info;

defined('MOODLE_INTERNAL') || die();

/**
 * Class mod_elediachecklist_renderer
 */
class mod_elediachecklist_renderer extends plugin_renderer_base {
    /**
    ...
    public function progress_bar($totalitems, $completeitems, $isrequired) {
        ...
        if ($isrequired) {
            $heading = get_string('percentcomplete', 'elediachecklist');
            $spanid = 'checklistprogressrequired';
        } else {
            $heading = get_string('percentcompleteall', 'elediachecklist');
     ...
     */
    public function elediachecklist_items($items, $useritems, $groupings, $intro, output_status $status, $progress, $student = null) {
        ...
        if ($status->is_viewother()) {
            echo '<h2>'.get_string('checklistfor', 'elediachecklist').' '.fullname($student, true).'</h2>';
            echo '&nbsp;';
            ...
            echo '<input type="submit" class="btn btn-secondary" name="viewall" value="'.get_string('viewall', 'elediachecklist').'" />';
            echo '</form>';
            ...
            if (!$status->is_editcomments()) {
                ...
                echo ' <input type="submit" class="btn btn-secondary" name="viewall" value="'.
                    get_string('addcomments', 'elediachecklist').'" />';
                echo '</form>';
            }
            echo '<form style="display: inline;" action="'.$thispageurl->out_omit_querystring().'" method="get">';
            echo html_writer::input_hidden_params($thispageurl);
            ...
            echo ' <input type="submit" class="btn btn-secondary" name="toggledates" value="'.
                get_string('toggledates', 'elediachecklist').'" />';
            echo '</form>';

            $strteacherdate = get_string('teacherdate', 'mod_elediachecklist');
            $struserdate = get_string('userdate', 'mod_elediachecklist');
            $strteachername = get_string('teacherid', 'mod_elediachecklist');
        }
        ...
        if (!$items) {
            print_string('noitems', 'elediachecklist');
        } else {
            $focusitem = false;
                ...
                if ($status->is_canaddown() && !$status->is_viewother()) {
                    ...
                    if ($status->is_addown()) {
                        ...
                        echo '<input type="submit" class="btn btn-secondary" name="submit" value="'.
                            get_string('addownitems-stop', 'elediachecklist').'" />';
                    } else {
                        echo '<input type="hidden" name="useredit" value="on" />';
                        echo '<input type="submit" class="btn btn-secondary" name="submit" value="'.
                            get_string('addownitems', 'elediachecklist').'" />';
                    }
                    echo '</form>';
                }
            ...
            if ($status->is_teachermarklocked()) {
                echo '<p class="checklistwarning">'.get_string('lockteachermarkswarning', 'elediachecklist').'</p>';
            ...
            foreach ($items as $item) {
                ...
                if ($status->is_addown()) {
                    ...
                    $title = get_string('additemalt', 'elediachecklist');
                    echo $this->output->pix_icon('add', $title, 'mod_elediachecklist', ['title' => $title]).'</a>';
                }
                        ...
                        while ($useritem && ($useritem->position == $item->position)) {
                            ...
                            if ($useritem->is_editme()) {
                                ...
                                echo '<input type="submit" class="btn btn-secondary" name="canceledititem" value="'.
                                    get_string('canceledititem', 'elediachecklist').'" />';
                                echo '</form>';
                                ...
                                if ($status->is_addown()) {
                                    ...
                                    $title = get_string('edititem', 'elediachecklist');
                                    echo $this->output->pix_icon('t/edit', $title, 'moodle', ['title' => $title]).'</a>';

                                    echo '&nbsp;<a href="'.$baseurl.'deleteitem" class="deleteicon">';
                                    $title = get_string('deleteitem', 'elediachecklist');
                                    echo $this->output->pix_icon('remove', $title, 'mod_elediachecklist', ['title' => $title]).'</a>';
                                }
                                if ($note != '') {
            ...
            if ($status->is_updateform()) {
                ...
                if ($status->is_viewother()) {
                    echo '&nbsp;<input type="submit" class="btn btn-secondary" name="save" value="'.
                        get_string('savechecks', 'mod_elediachecklist').'" />';
            ...
            if ($status->is_addown()) {
                ...
                echo 'if (confirm("'.get_string('confirmdeleteitem', 'elediachecklist').'")) { window.location = url; } ';
                echo '} ';
     ...
     */
    protected function elediachecklist_item_link(checklist_item $item) {
        $out = '';
        if ($url = $item->get_link_url()) {
            ...
            switch ($item->get_link_type()) {
                case checklist_item::LINK_MODULE:
                    $icon = $this->output->pix_icon('follow_link', get_string('linktomodule', 'elediachecklist'), 'mod_elediachecklist');
                    break;
                case checklist_item::LINK_COURSE:
                    $icon = $this->output->pix_icon('i/course', get_string('linktocourse', 'elediachecklist'));
                    break;
                case checklist_item::LINK_URL:
                    $icon = $this->output->pix_icon('follow_link', get_string('linktourl', 'elediachecklist'), 'mod_elediachecklist');
     ...
     * @param output_status $status
     */
    public function elediachecklist_edit_items($items, $status) {
        ...
        if ($status->is_autoupdatewarning()) {
            switch ($status->get_autoupdatewarning()) {
                ...
                case ELEDIACHECKLIST_MARKING_TEACHER:
                    ...
                    break;
                default:
                    echo '<p class="checklistwarning">'.get_string('autoupdatewarning_both', 'elediachecklist').'</p>';
                    break;
            }
        }

        // Start the ordered list of checklist items.
        $attr = ['class' => 'elediachecklist'];
        ...
        if ($items) {
            ...
            if ($status->is_autopopulate()) {
                echo html_writer::empty_tag('input', array(
                    ...
                    'value' => get_string('showhidechecked', 'elediachecklist'),
            ...
            foreach ($items as $item) {
                ...
                if ($item->is_optional()) {
                    $title = get_string('optionalitem', 'elediachecklist');
                    echo '<a href="'.$itemurl->out(true, array('action' => 'makeheading')).'">';
                    echo $this->output->pix_icon('empty_box', $title, 'mod_elediachecklist', ['title' => $title]).'</a>&nbsp;';
                    $optional = ' class="itemoptional '.$itemcolour.$autoclass.'" ';
                } else if ($item->is_heading()) {
                    if ($item->hidden) {
                        $title = get_string('headingitem', 'elediachecklist');
                        echo $this->output->pix_icon('no_box', $title, 'mod_elediachecklist', ['title' => $title]).'&nbsp;';
                    ...
                    $title = get_string('requireditem', 'elediachecklist');
                    echo $this->output->pix_icon('tick_box', $title, 'mod_elediachecklist', ['title' => $title]).'&nbsp;';
                    $optional = ' class="'.$itemcolour.$autoclass.' itemdisabled"';
                } else {
                    $title = get_string('requireditem', 'elediachecklist');
                    echo '<a href="'.$itemurl->out(true, array('action' => 'makeoptional')).'">';
                    echo $this->output->pix_icon('tick_box', $title, 'mod_elediachecklist', ['title' => $title]).'</a>&nbsp;';
                ...
                if ($item->is_editme()) {
                    ...
                    // Item colour.
                    if (!empty(get_config('mod_elediachecklist', 'showcolorchooser'))) {
                        echo '<a href="' . $itemurl->out(true, array('action' => 'nextcolour')) . '">';
                        $title = get_string('changetextcolour', 'elediachecklist');
                        echo $this->output->pix_icon($nexticon, $title, 'mod_elediachecklist', ['title' => $title]) . '</a>';
                    }
                    ...
                    if (!$autoitem && $item->position > 1) {
                        ...
                        $title = get_string('moveitemup', 'elediachecklist');
                    ...
                    if (!$autoitem && $item->position < $lastitem) {
                        ...
                        $title = get_string('moveitemdown', 'elediachecklist');
                    ...
                    $title = get_string('additemhere', 'elediachecklist');
                    echo $this->output->pix_icon('add', $title, 'mod_elediachecklist', ['title' => $title]).'</a>';
        ...
        if ($status->is_editdates()) {
            ...
            $editdatesstr = get_string('editdatesstop', 'mod_elediachecklist');
        } else {
            $editdatesurl->param('editdates', 'on');
            $editdatesstr = get_string('editdatesstart', 'mod_elediachecklist');
        }
        ...
        if (!$status->is_autopopulate() && $hasauto) {
            ...
            echo $this->output->single_button($removeautourl, get_string('removeauto', 'mod_elediachecklist'));
        }
...
ENDSCRIPT;
    ...
    protected function edit_link_form(output_status $status, $item = null) {
        ...
        $attr = [
            ...
            'placeholder' => get_string('enterurl', 'mod_elediachecklist'),
            'class' => 'form-control',
        ];
        $out .= html_writer::empty_tag('input', $attr);
        ...
        $out .= html_writer::checkbox(
            ...
            get_string('openlinkinnewwindow', 'mod_elediachecklist'),
            $attr
        );
